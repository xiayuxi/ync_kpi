		   	...
		   	<tbody id="tags"> 
			   	 <tr>
						...
						<select id="employe_id" name="efficiencyRecordBo[0].employeId" class="span2 required">
			   	   	...
			   	   	<td>
			   	   		周期：<input class="Wdate input-small required" readonly="readonly" onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd ',minDate:'%y-%M-%d',maxDate:'#F{$dp.$D(\'efficiencyRecordBo[0].planEndTime\') || \'%y-%M-%ld\' }' })"
						type="text" placeholder="—请选择—" id="efficiencyRecordBo[0].planBeginTime" name="efficiencyRecordBo[0].planBeginTime" />
						-
						<input class="Wdate input-small required" readonly="readonly" onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd ',minDate:'#F{$dp.$D(\'efficiencyRecordBo[0].planBeginTime\') || \'%y-%M-%d\' }'  ,maxDate:'%y-%M-%ld'})" 
